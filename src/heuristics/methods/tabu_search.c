...
static bool tabu_list_is_allowed(tabu_list *tabu, neighbourhood_swap_move *mv, int time) {
    CRDSQT(tabu->model);
...
static void tabu_list_insert(tabu_list *tabu, neighbourhood_swap_move *mv,
                             int time) {
...
void tabu_search(heuristic_solver_state *state, void *arg) {
    ...
    verbose("TS.clear_on_new_best = %s", BOOL_TO_STR(params->clear_on_new_best));
    ...
    while (state->current_cost > 0 && idle < params->max_idle) {
        ...
        while (neighbourhood_swap_iter_next(&swap_iter, &swap_mv)) {
            neighbourhood_swap(state->current_solution, &swap_mv,
                               ...
                               &swap_result);
            n_banned += tabu_list_is_allowed(&tabu, &swap_mv, iter) ? 0 : 1;
            n_side += swap_result.delta_cost == 0 ? 1 : 0;

            if (swap_result.feasible &&
                    swap_result.delta_cost <= best_swap_cost &&
                    (state->current_cost + swap_result.delta_cost < state->best_cost ||
                    tabu_list_is_allowed(&tabu, &swap_mv, iter))) {
        ...
        if (best_swap_cost != INT_MAX) {
            ...
            if (new_best && params->clear_on_new_best)
                tabu_list_clear(&tabu);
            tabu_list_insert(&tabu, &swap_mv, iter);
        }
        ...
        if (idle && idle % (params->max_idle / 10) == 0) {
            verbose2("TS: idle progress %d/%d (%g%%)", idle, params->max_idle,
                     (double) 100 * idle / params->max_idle);
            verbose2("TS: current = %d | local_best = %d | global_best = %d | it = %d | idle = %d/%d | # banned = %d/%d | # side = %d/%d",
                  state->current_cost, local_best_cost, state->best_cost, iter, idle, params->max_idle, n_banned, swap_iter.i, n_side, swap_iter.i);
        }
