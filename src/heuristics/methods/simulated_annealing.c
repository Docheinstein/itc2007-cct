...
static bool simulated_annealing_accept(heuristic_solver_state *state,
                                       ...
                                       double temperature) {
    if (state->current_cost + swap_result->delta_cost < state->best_cost)
        return true; // always accept best solutions

    double p = SA_ACCEPTANCE(swap_result->delta_cost, temperature);
...
void simulated_annealing(heuristic_solver_state *state, void *arg) {
    ...
    verbose("SA.temperature = %g", params->temperature);
