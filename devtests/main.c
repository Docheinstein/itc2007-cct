...
void test_finder(model *model, double ranking_randomness, int trials) {
    ...
    print("%s(r=%.2f)  %d/%d  (%.2f%%) feasible solutions %s found in %ldms (%ld sol/ms)",
          model->_filename, ranking_randomness,
          successes, trials,  (double) 100 * successes / trials,
          tmp, end - start, trials / (end - start));
...
void tests() {
    test_finder_single("datasets/comp03.ctt", 0.33, 0.33, 0.00, 100);
